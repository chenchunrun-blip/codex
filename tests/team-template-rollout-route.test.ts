import { beforeEach, describe, expect, it, jest } from "@jest/globals"

jest.mock("@/lib/auth/rbac", () => ({
  requireAuthApi: jest.fn(async () => ({
    user: { id: "user_admin", email: "admin@example.com" }
  }))
}))

jest.mock("@/lib/db", () => ({
  db: {
    teamMember: {
      findFirst: jest.fn()
    },
    template: {
      findFirst: jest.fn()
    },
    project: {
      findMany: jest.fn()
    },
    projectMember: {
      findMany: jest.fn()
    },
    file: {
      findFirst: jest.fn(),
      create: jest.fn()
    },
    activityLog: {
      create: jest.fn()
    }
  }
}))

const { db } = require("@/lib/db")
const dbMock = db as Record<string, any>
const { POST: applyTemplatePost } = require("@/app/api/teams/[id]/starter-files/apply-template/route")

describe("Team Template Rollout Route", () => {
  beforeEach(() => {
    jest.clearAllMocks()
  })

  it("rolls out template to editable projects", async () => {
    dbMock.teamMember.findFirst.mockResolvedValue({ role: "ADMIN" })
    dbMock.template.findFirst.mockResolvedValue({
      id: "tpl_1",
      name: "QA Test Plan (IT Delivery)",
      category: "SOLUTION_DESIGN",
      content: "# qa"
    })
    dbMock.project.findMany.mockResolvedValue([
      { id: "p1", name: "Alpha" },
      { id: "p2", name: "Beta" }
    ])
    dbMock.projectMember.findMany.mockResolvedValue([
      { projectId: "p1", role: "EDITOR" },
      { projectId: "p2", role: "VIEWER" }
    ])
    dbMock.file.findFirst.mockResolvedValue(null)
    dbMock.file.create.mockResolvedValue({ id: "f1" })
    dbMock.activityLog.create.mockResolvedValue({})

    const req = new Request("http://localhost/api/teams/team_1/starter-files/apply-template", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        templateId: "tpl_1",
        dryRun: false
      })
    })
    const res = await applyTemplatePost(req, { params: Promise.resolve({ id: "team_1" }) })
    const data = await res.json()

    expect(res.status).toBe(200)
    expect(data.teamId).toBe("team_1")
    expect(data.templateId).toBe("tpl_1")
    expect(data.successCount).toBe(1)
    expect(data.failedCount).toBe(1)
    expect(data.results.find((item: any) => item.projectId === "p2")?.message).toBe(
      "INSUFFICIENT_PROJECT_PERMISSIONS"
    )
    expect(dbMock.activityLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          action: "TASK_UPDATED",
          metadata: expect.objectContaining({
            type: "TEAM_TEMPLATE_ROLLOUT_APPLIED",
            teamId: "team_1",
            templateId: "tpl_1"
          })
        })
      })
    )
  })

  it("supports dry run", async () => {
    dbMock.teamMember.findFirst.mockResolvedValue({ role: "ADMIN" })
    dbMock.template.findFirst.mockResolvedValue({
      id: "tpl_1",
      name: "QA Test Plan (IT Delivery)",
      category: "SOLUTION_DESIGN",
      content: "# qa"
    })
    dbMock.project.findMany.mockResolvedValue([{ id: "p1", name: "Alpha" }])
    dbMock.projectMember.findMany.mockResolvedValue([{ projectId: "p1", role: "ADMIN" }])
    dbMock.file.findFirst.mockResolvedValue(null)
    dbMock.activityLog.create.mockResolvedValue({})

    const req = new Request("http://localhost/api/teams/team_1/starter-files/apply-template", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        templateId: "tpl_1",
        dryRun: true
      })
    })
    const res = await applyTemplatePost(req, { params: Promise.resolve({ id: "team_1" }) })
    const data = await res.json()

    expect(res.status).toBe(200)
    expect(data.dryRun).toBe(true)
    expect(data.results[0].wouldCreateCount).toBe(1)
    expect(dbMock.file.create).not.toHaveBeenCalled()
  })
})
