...
export function SaveStarterPacksButton() {
  ...
  const save = async () => {
    ...
    setError(null)
    try {
      const response = await fetch("/api/templates/starter-packs/save", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to save starter packs"))
      }
      if (payload?.file?.id) {
      ...
      setError(err instanceof Error ? err.message : "Failed to save starter packs")
    } finally {
      setLoading(false)
    }
  }
      ...
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Save Template Starter Packs</h3>
            <p className="mt-1 text-sm text-gray-600">Save starter-packs catalog into a project file.</p>
            {error && (
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="starter-packs-catalog-custom.md"
                  className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
                />
              </div>
            </div>
