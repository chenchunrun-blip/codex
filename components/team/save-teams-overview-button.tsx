...
interface SaveTeamsOverviewButtonProps {
  endpoint?: string
}

export function SaveTeamsOverviewButton({
  endpoint = "/api/teams/overview/save"
}: SaveTeamsOverviewButtonProps) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to save teams overview"))
      }
      if (payload?.file?.id) {
      ...
      setError(err instanceof Error ? err.message : "Failed to save teams overview")
    } finally {
      setLoading(false)
    }
  }
      ...
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Save Teams Overview</h3>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="teams-overview-custom.md"
                  className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
                />
              </div>
            </div>
