...
interface SaveTeamRolloutSummaryButtonProps {
  teamId: string
  projects: ProjectItem[]
}

export function SaveTeamRolloutSummaryButton({
  teamId,
  projects
}: SaveTeamRolloutSummaryButtonProps) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to save team rollout summary"))
      }
      if (payload?.file?.id) {
      ...
      setError(err instanceof Error ? err.message : "Failed to save team rollout summary")
    } finally {
      setLoading(false)
    }
  }
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            ...
            <p className="mt-1 text-sm text-gray-600">Save summary KPI report into a project file.</p>
            {error && (
