...
interface SaveWorkspaceReportButtonProps {
  endpoint?: string
}
...
export function SaveWorkspaceReportButton({
  endpoint = "/api/workspace/report/save"
}: SaveWorkspaceReportButtonProps) {
  ...
  const saveReport = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to save workspace report"))
      }
      ...
      setError(error instanceof Error ? error.message : "Failed to save workspace report")
    } finally {
      setLoading(false)
    }
  }
      ...
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Save Workspace Report</h3>
            <p className="mt-1 text-sm text-gray-600">
              Choose target project and optional report filename.
            </p>
            {error && (
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="workspace-report-custom.md"
                  className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
                />
              </div>
            </div>
