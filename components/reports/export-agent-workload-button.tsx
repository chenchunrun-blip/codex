...
export function ExportAgentWorkloadButton() {
  ...
  const runExport = async () => {
    ...
    setError(null)
    try {
      const parsedHours = Math.max(1, Math.min(168, Number(hours) || 24))
      ...
      const response = await fetch(`/api/agents/workload?${params.toString()}`)
      if (!response.ok) {
        const payload = await response.json().catch(() => ({}))
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to export agent workload"))
      }
      ...
      setError(err instanceof Error ? err.message : "Failed to export agent workload")
    } finally {
      setExporting(false)
    }
  }
      ...
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Export Agent Workload</h3>
            <p className="mt-1 text-sm text-gray-600">Configure source scope and hours window.</p>
            {error && (
