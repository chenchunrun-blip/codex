...
interface ExportTeamStarterHistoryButtonProps {
  defaultTeamId?: string
}

export function ExportTeamStarterHistoryButton({ defaultTeamId }: ExportTeamStarterHistoryButtonProps = {}) {
  ...
  const runExport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/teams/starter-files/history?${params.toString()}`)
      if (!response.ok) {
        const payload = await response.json().catch(() => ({}))
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to export team rollout history"))
      }
      ...
      setError(err instanceof Error ? err.message : "Failed to export team rollout history")
    } finally {
      setExporting(false)
    }
  }
      ...
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Export Team Rollout History</h3>
            <p className="mt-1 text-sm text-gray-600">Choose team and record limit.</p>
            {error && (
