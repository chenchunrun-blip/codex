...
interface SaveOperationsStatusButtonProps {
  endpoint?: string
  defaultTargetProjectId?: string
  defaultSourceProjectId?: string
}

export function SaveOperationsStatusButton({
  endpoint = "/api/operations/status/save",
  defaultTargetProjectId,
  defaultSourceProjectId
}: SaveOperationsStatusButtonProps) {
    ...
    try {
      ...
      if (items.length > 0) {
        if (defaultTargetProjectId && items.some((item) => item.id === defaultTargetProjectId)) {
          setProjectId(defaultTargetProjectId)
        } else {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to save operations status"))
      }
      if (payload?.file?.id) {
      ...
      setError(err instanceof Error ? err.message : "Failed to save operations status")
    } finally {
      setLoading(false)
    }
  }
      ...
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Save Operations Status Report</h3>
            <p className="mt-1 text-sm text-gray-600">Choose target project and optional source scope.</p>
            {error && (
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="operations-status-custom.md"
                  className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
                />
              </div>
            </div>
