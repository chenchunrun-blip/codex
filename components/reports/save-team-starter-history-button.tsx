...
interface SaveTeamStarterHistoryButtonProps {
  defaultTeamId?: string
  defaultTargetProjectId?: string
}

export function SaveTeamStarterHistoryButton({
  defaultTeamId,
  defaultTargetProjectId
}: SaveTeamStarterHistoryButtonProps = {}) {
  ...
  const fetchOptions = async () => {
    ...
    setError(null)
    try {
      const [teamItems, projectItems] = await Promise.all([
        fetchTeamOptionsCached(),
        fetchProjectOptionsCached()
      ])
      ...
      const preferredTeamId =
        defaultTeamId && teamItems.some((item) => item.id === defaultTeamId) ? defaultTeamId : teamItems[0]?.id || ""
      const preferredProjectId =
        defaultTargetProjectId && projectItems.some((item) => item.id === defaultTargetProjectId)
          ? defaultTargetProjectId
          : projectItems[0]?.id || ""
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to save team starter history"))
      }
      if (payload?.file?.id) {
      ...
      setError(err instanceof Error ? err.message : "Failed to save team starter history")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <>
      ...
      >
        Save Team Rollout History
      </button>
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Save Team Rollout History</h3>
            <p className="mt-1 text-sm text-gray-600">Choose team and target project for the rollout report file.</p>
            {error && (
