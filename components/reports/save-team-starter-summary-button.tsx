...
interface SaveTeamStarterSummaryButtonProps {
  defaultTeamId?: string
  defaultTargetProjectId?: string
}

export function SaveTeamStarterSummaryButton({
  defaultTeamId,
  defaultTargetProjectId
}: SaveTeamStarterSummaryButtonProps = {}) {
  ...
  const fetchProjectsByTeam = async (selectedTeamId: string) => {
    ...
    const items = Array.isArray(payload)
      ? payload.map((item: { id: string; name: string }) => ({ id: item.id, name: item.name }))
      : []
    setProjects(items)
  ...
  const save = async () => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/teams/${teamId}/starter-files/summary/save`, {
        method: "POST",
      ...
      const payload = await response.json().catch(() => ({}))
      if (!response.ok) {
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to save team starter rollout summary"))
      }
      if (payload?.file?.id) {
      ...
      setError(err instanceof Error ? err.message : "Failed to save team starter rollout summary")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <>
      ...
      >
        Save Team Rollout Summary
      </button>
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Save Team Rollout Summary</h3>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="team-rollout-summary-custom.md"
                  className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
                />
              </div>
            </div>
