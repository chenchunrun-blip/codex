...
export function SaveProjectWorkspaceReportButton() {
  ...
  const save = async () => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/projects/${selectedProjectId}/workspace-report/save`, {
        method: "POST"
      })
      const payload = await response.json().catch(() => ({}))
      if (!response.ok) {
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to save project workspace report"))
      }
      if (payload?.file?.id) {
      ...
      setError(err instanceof Error ? err.message : "Failed to save project workspace report")
    } finally {
      setLoading(false)
    }
  }
      ...
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Save Project Workspace Report</h3>
            <p className="mt-1 text-sm text-gray-600">Choose project and save current workspace snapshot.</p>
            {error && (
