...
interface ExportFilesInventoryHubButtonProps {
  endpoint?: string
}

export function ExportFilesInventoryHubButton({
  endpoint = "/api/files/inventory"
}: ExportFilesInventoryHubButtonProps) {
  ...
  const runExport = async () => {
    ...
    try {
      ...
      anchor.href = url
      anchor.download = `files-inventory-${scope}-${stamp}.md`
      ...
      setError(err instanceof Error ? err.message : "Failed to export files inventory")
    } finally {
      setExporting(false)
    }
  }
      ...
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Export Files Inventory</h3>
