...
interface SaveProjectsPortfolioButtonProps {
  endpoint?: string
  sourceProjectIds?: string[]
}

export function SaveProjectsPortfolioButton({
  endpoint = "/api/projects/portfolio/save",
  sourceProjectIds = []
}: SaveProjectsPortfolioButtonProps) {
    ...
    try {
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          sourceProjectIds: sourceProjectIds.length > 0 ? sourceProjectIds : undefined,
          fileName: fileName.trim() || undefined
        })
      })
      const payload = await response.json().catch(() => ({}))
      if (!response.ok) {
        throw new Error(mapReportApiErrorFromPayload(payload, "Failed to save projects portfolio"))
      }
      if (payload?.file?.id) {
      ...
      setError(err instanceof Error ? err.message : "Failed to save projects portfolio")
    } finally {
      setLoading(false)
    }
  }
      ...
      {showDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900">Save Projects Portfolio</h3>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="projects-portfolio-custom.md"
                  className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
                />
              </div>
            </div>
